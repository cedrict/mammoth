...
subroutine pmgmres_ilu_cr ( n, nz_num, ia, ja, a, x, rhs, itr_max, mr, &
  tol_abs, tol_rel , itr_used, iunit, use_prec)
...
!    Tim Kelley, Iterative Methods for Linear and Nonlinear Equations,
!    SIAM, 2004, ISBN: 0898713528,
!    Yousef Saad, Iterative Methods for Sparse Linear Systems,
!    Second Edition, SIAM, 2003, ISBN: 0898715342,
!
!  Parameters:
...
!
  ...
  integer ( kind = 4 ), intent(out) :: itr_used
  integer ( kind = 4 ), intent(in)  :: iunit
  logical             , intent(in)  :: use_prec 
...
!-----------------------------
  ...
  integer ( kind = 4 ), dimension(:), allocatable ::  ua
  real ( kind = 8 ), dimension(:), allocatable ::  c,g,l,r,s,y
  real ( kind = 8 ), dimension(:,:), allocatable :: h,v
...
!-----------------------
  ...
  if (use_prec)  call ilu_cr ( n, nz_num, ia, ja, a, ua, l )
  ...
  do itr = 1, itr_max
    ...
    if (use_prec) call lus_cr ( n, nz_num, ia, ja, l, ua, r, r )

    rho = sqrt ( dot_product ( r, r ) )

    !write ( iunit, '(a,i4,a,es15.6)' ) '  ITR = ', itr, '  Residual = ', rho
    ...
    do k = 1, mr
      ...
      if (use_prec) call lus_cr ( n, nz_num, ia, ja, l, ua, v(1:n,k+1), v(1:n,k+1) )
      ...
      !write ( iunit, '(i4,es10.4)' ) itr_used, rho  
      write ( iunit, '(a,i4,a,g14.6,a,g14.6)' ) 'K= ', k, ' Residual= ', rho, ' rho_tol= ',rho_tol
      call flush(iunit)
    ...
    if ( rho <= rho_tol .and. rho <= tol_abs ) then
       !print *,'outer',k,rho,rho_tol
       exit
    end if

  end do

  !write ( *, '(a,i5,a,g10.3)' )    'Iterations=', itr_used,' Residual= ', rho
